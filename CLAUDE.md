...
WordSail is an automated WordPress hosting infrastructure using Ansible. It consists of:
...
**Note**: Users typically interact via the CLI (`wordsail` command), but you can run Ansible directly for testing or advanced usage.
...
**Roles Architecture:**
- **bootstrap**: Creates `wordsail` system user, installs base packages, certbot, fail2ban, redis
- **database**: Installs MariaDB, creates `wordsailbot` admin user
...
**Module Path:** `github.com/wordsail/cli`
...
- `internal/installer/`: Setup logic for copying ansible files to ~/.wordsail/
...
**Ansible Integration:**
- `wordsail init` copies ansible/ directory to `~/.wordsail/ansible/` on first run
- CLI detects ansible location (user's ~/.wordsail/ansible/ or system install path)
...
- Ansible paths resolved in order: `~/.wordsail/ansible/` → `/usr/local/share/wordsail/ansible/` → relative path (dev mode)
- `wordsail init` must be run once after installation to copy ansible files locally
