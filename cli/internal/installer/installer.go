...
const (
	wordsailDir = ".wordsail"
	ansibleDir  = "ansible"
)

// GetWordsailDir returns the path to ~/.wordsail/
func GetWordsailDir() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, wordsailDir)
}

// GetAnsibleDir returns the path to ~/.wordsail/ansible/
func GetAnsibleDir() string {
	return filepath.Join(GetWordsailDir(), ansibleDir)
}

// IsInitialized checks if ~/.wordsail/ansible/ exists and has content
...
func DetectAnsibleSource() (string, error) {
	...
	systemPath := "/usr/local/share/wordsail/ansible"
...
// Initialize sets up the ~/.wordsail directory and copies ansible files
func Initialize() error {
	wordsailPath := GetWordsailDir()
	ansiblePath := GetAnsibleDir()

	// Create ~/.wordsail/ directory
	if err := os.MkdirAll(wordsailPath, 0755); err != nil {
		return fmt.Errorf("failed to create %s: %w", wordsailPath, err)
	}
...
// Checks in order: ~/.wordsail/ansible/, /usr/local/share/wordsail/ansible/, relative path
